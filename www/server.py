...
async def send_realtime_data():
    ...
    while True:
        if ws_clients:
            msg = f"Data {i} | Waktu: {time.strftime('%H:%M:%S')}"
...
async def start_ws_server():
    async with websockets.serve(ws_handler, HOST, PORT_WS):
        print(f"🌐 WebSocket jalan di ws://{HOST}:{PORT_WS}")
        await send_realtime_data()

if __name__ == "__main__":
    threading.Thread(target=run_advanced_server, daemon=True).start()
